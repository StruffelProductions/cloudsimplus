 ...
 */
public class CloudletTaskSchedulerMicroservice implements CloudletTaskScheduler {
     ...
     */
    public CloudletTaskSchedulerMicroservice() {
        super();
    ...
    public void processCloudletTasks(final Cloudlet cloudlet, final long partialFinishedMI) {
    	//LOGGER.trace("{} PROCESSING TASKS", Instant.now() );
        if (cloudlet.isFinished() || isNotNetworkCloudlet(cloudlet)) {
            return;
        }
        
        // TODO: Dieser Check muss für jede Gruppe ausgeführt werden
    ...
    private void updateWaitTasks(final NetworkCloudlet cloudlet) {
    	List<CloudletTask> allCurrentWaitTasks = cloudlet.getAllCurrentTasks().stream().filter(CloudletTask::isWaitTask).collect(Collectors.toList());
    	
    	for(CloudletTask t : allCurrentWaitTasks) {
    		CloudletWaitTask wt = (CloudletWaitTask) t;
    		wt.finishIfPauseIsOver();
    	}
    }

    private void updateExecutionTasks(final NetworkCloudlet cloudlet, final long partialFinishedMI) {
    	
    	//LOGGER.debug("Executing {} MIs",partialFinishedMI);
    ...
    private void updateNetworkTasks(final NetworkCloudlet cloudlet) {
    	...
    	//LOGGER.trace("CLOUDLET {} IS CURRENTLY RUNNING {} THREADS",cloudlet.getId(),cloudlet.getAllCurrentTasks().size());
    	
    	for(CloudletTask task : allCurrentNetworkingTasks) {

    		//LOGGER.trace("UPDATE NETWORK ON CLOUDLET {} TASK {} IN GROUP {}",task.getCloudlet().getId(),task.getId(),task.getTaskGroup().getId());
    ...
    public boolean isTimeToUpdateCloudletProcessing(final Cloudlet cloudlet) {
        ...
        boolean oneCurrentExecutionTaskExists = false;
        for(CloudletTask t : ((NetworkCloudlet)cloudlet).getAllCurrentTasks()) {
        	oneCurrentExecutionTaskExists = t.isExecutionTask() || oneCurrentExecutionTaskExists;
        }

        //return ((NetworkCloudlet)cloudlet).getCurrentTask().filter(CloudletTask::isExecutionTask).isPresent();
        return oneCurrentExecutionTaskExists;
    }
    ...
    private List<VmPacket> getPacketsSentToCloudlet(final CloudletReceiveTask receiveTask) {
        ...
        return pktsFromExpectedSenderVm
                ...
                .filter(pkt -> ((TaskPacket) pkt).getReceiverTask().equals(receiveTask))
